...
#UI
ui <- fluidPage(
  titlePanel("The locations of the Victoria road restrictions"),
  sidebarLayout(
    position = 'right',
    sidebarPanel(
      selectInput("selected_road",
                  "ROAD Category",
                  choice=truckable_dataset1$HV_CATEGORY, 
...
server <- function(input,output,session){
  ...
  observeEvent(input$selected_road,
               output$mymap <- renderLeaflet({
                 ...
                 truckable_dataset1 <- filter(truckable_dataset1,truckable_dataset1$HV_CATEGORY==input$selected_road)
                 leaflet(truckable_dataset1) %>%
                   addTiles() %>%
                   addCircleMarkers(~LONGITUDE,~LATITUDE,#radius = ~Scale,#popup = ~as.character(Hourly_counts1),
                                    popup = paste('Road Name: ',truckable_dataset1$ROAD_NAME,"</br>",
